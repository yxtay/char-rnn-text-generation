...
class LoggerCallback(Callback):
    ...
    """
    def __init__(self, text, model, log_session_boundaries=True):
        super(LoggerCallback, self).__init__()
        self.text = text
        self.log_session_boundaries = log_session_boundaries
    ...
    def on_train_begin(self, logs=None):
        if not self.log_session_boundaries:
            return
...
def train_main(args):
    ...
    # make and clear checkpoint directory (fresh run only; keep files when resuming)
    ...
    if not multi_corpus:
        ...
        model.reset_states()
        model.fit_generator(
            batch_generator(encode_text(text), args.batch_size, args.seq_len, one_hot_labels=True),
            num_batches, args.num_epochs, callbacks=callbacks)
        return model

    _, text0 = corpus_for_training_epoch(text_paths, 0)
    logger.info("corpus length (first file): %s.", len(text0))
    logger_cb = LoggerCallback(text0, model, log_session_boundaries=False)
    callbacks = [
        ModelCheckpoint(args.checkpoint_path, verbose=1, save_best_only=False),
        TensorBoard(log_dir, write_graph=True, embeddings_freq=1,
                    embeddings_metadata={"embedding_1": os.path.abspath(os.path.join("data", "id2char.tsv"))}),
    ...
    for i in range(args.num_epochs):
        ...
        model.fit_generator(
            batch_generator(encode_text(text), args.batch_size, args.seq_len, one_hot_labels=True),
            num_batches, 1, initial_epoch=i, callbacks=callbacks)
    duration_train = time.time() - time_train
    logger.info("end of training, duration: %ds.", duration_train)
    logger_cb.inference_model.set_weights(model.get_weights())
    seed = generate_seed(logger_cb.text)
    generate_text(logger_cb.inference_model, seed, 1024, 3)
    return model
